...
const SUCCESS_MESSAGES: Record<string, string> = {
  created: "Haber eklendi.",
  updated: "Haber güncellendi.",
  deleted: "Haber silindi.",
};
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-zinc-900">Haberler</h2>
        <Link
          href={`/dashboard/${site.id}/news/new`}
      ...
      {posts.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {posts.map((post) => (
            <div key={post.id} className="flex items-center gap-4 rounded-xl border border-zinc-200 bg-white p-4">
              {post.coverImageUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={post.coverImageUrl} alt={post.title} className="h-12 w-12 rounded object-cover" />
              ) : (
              ...
              <span
                className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                  post.published ? "bg-green-100 text-green-700" : "bg-zinc-100 text-zinc-500"
                }`}
              >
                {post.published ? "Yayında" : "Taslak"}
              </span>
              <Link
                href={`/dashboard/${site.id}/news/${post.id}`}
                className="rounded-md border border-zinc-300 px-3 py-1.5 text-sm font-medium hover:bg-zinc-50"
              >
                Düzenle
              </Link>
              <form action={deleteNewsPost.bind(null, site.id, post.id)}>
                <button
                  type="submit"

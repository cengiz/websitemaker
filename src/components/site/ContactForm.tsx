    ...
    <form action={apiUrl} method="post" onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="flex flex-col gap-1">
          ...
          <input name="name" required className="site-input" />
        </label>
        <label className="flex flex-col gap-1">
          <span className="text-sm font-medium">E-posta</span>
          <input name="email" type="email" required className="site-input" />
        </label>
      </div>
      <label className="flex flex-col gap-1">
        <span className="text-sm font-medium">
          Telefon <span className="font-normal text-[var(--site-muted)]">(isteğe bağlı)</span>
        </span>
        <input name="phone" type="tel" className="site-input" />
      </label>
      <label className="flex flex-col gap-1">
        <span className="text-sm font-medium">Mesaj</span>
        <textarea name="message" required rows={5} className="site-input" />
      </label>
      {state === "error" && (
      ...
      <button
        ...
        className="site-btn-primary self-start disabled:opacity-60"
      >
